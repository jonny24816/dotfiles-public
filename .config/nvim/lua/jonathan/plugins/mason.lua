return {
    ...
    },
    {
        ...
        config = function()
            ...
            vim.keymap.set("n", "<leader>sd", vim.lsp.buf.hover, { desc = "[C]ode [H]over Documentation" })
            -- Set vim motion for <Space> + c + d to go where the code/variable under the cursor was defined
            vim.keymap.set("n", "<leader>gd", vim.lsp.buf.definition, { desc = "[C]ode Goto [D]efinition" })
            -- Set vim motion for <Space> + c + a for display code action suggestions for code diagnostics in both normal and visual mode
            vim.keymap.set({ "n", "v" }, "<leader>.", vim.lsp.buf.code_action, { desc = "[C]ode [A]ctions" })
            ...
            vim.keymap.set("n", "<leader>rb", vim.lsp.buf.rename, { desc = "[C]ode [R]ename" })
